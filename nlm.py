# -*- coding: utf-8 -*-
"""
Python Code for the Non local filter 
proposed for A. Buades, B. Coll and J.M. Morel in
        ...
        for j in range(0,n) :
            ...
            for r in range(rmin,rmax+1) :
                for s in range(smin,smax+1) :
                    ...
                    # Use L2-norm
                    temp = np.reshape(np.square(W1-W2), (2*f+1)*(2*f+1) )
...
sigma = 0.1            # Controls amount of Noise
[m, n]  = img.shape
img_noisy = img + sigma * np.random.randn(m,n)   # Gaussian Noise

# NLM Denoising
img_denoised = nlm(img_noisy,5,2,sigma)
